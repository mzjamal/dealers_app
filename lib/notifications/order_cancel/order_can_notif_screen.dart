...
import './order_can_noti_item.dart';
import '../notifications_tiles_screen.dart';
import '../../globals.dart';
import '../../colors.dart';

class OrderCancelNotifScreen extends StatefulWidget {
  static const routeName = '/ordCanNotif';

  @override
  _OrderCancelNotifScreenState createState() => _OrderCancelNotifScreenState();
}

class _OrderCancelNotifScreenState extends State<OrderCancelNotifScreen> {
  var _isInit = true;
  List<OrderCancelNotifItem> cancelOrdItems = [];
  ...
  int _rowId = 1;
  final String url =
      'https://ffcportal.ffc.com.pk:8856/sap/opu/odata/sap/ZSDAORDNOTCAN_SRV/ZSDAOrdNotCan?sap-client=500&\$filter=DEALER eq \'' +
          Globals.dealerCode +
          '\'&\$format=json';
  var postURL =
  ...
  Future<List<OrderCancelNotifItem>> _getJsonData() async {
    ...
    try {
      ...
      xdata.forEach((element) {
        ...
        if (element['RECTYP'] == 'NEW') {
          OrderCancelNotifItem newOrdItem = OrderCancelNotifItem(
            ...
            cancelDate: element['CANCDATE'],
          );
          //print('hello');
          cancelOrdItems.add(newOrdItem);
                        ...
                        if (snapshot.data == null) {
                          ...
                          if (cancelOrdItems.isEmpty) {
                            return Container(
                              ...
                              child: Center(
                                child: Text(
                                  'منسوخ شدہ آرڈر کے بارے میں فی الحال کوئی اطلاع نہیں ہے۔',
                                  textAlign: TextAlign.center,
                                  style: TextStyle(
                                    fontSize: 26,
                                    fontFamily: 'Urdu',
